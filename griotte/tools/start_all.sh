...
tmux new-window -t griotte:1 -n Scheduler "sudo PYTHONPATH=${PWD}/griotte/lib:$PYTHONPATH PATH=${PWD}/griotte/bin:$PATH griotte/bin/subsystem scheduler --logging=$LOGGING || /bin/bash"
...
# tmux new-window -t griotte:7 -n RFID "griotte/bin/rfid --logging=$LOGGING; /bin/bash"

# tmux new-window -t griotte:8 -n DMX "griotte/bin/dmx --logging=$LOGGING; /bin/bash"
